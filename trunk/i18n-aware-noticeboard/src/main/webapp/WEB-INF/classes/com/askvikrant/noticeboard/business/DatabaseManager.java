...
public class DatabaseManager {
	...
	public DatabaseManager() throws Exception{
		try {
			...
			if (pool == null)
				throw new Exception("Unknown DataSource 'jdbc/nbpool'");
	...
	public String getNextAvailablePrimaryKeyValue(String tableName) {
		...
		try {
			PreparedStatement stmt = conn
					.prepareStatement("select id_value from _last_used_id where table_name = ?  ");
			...
			//			System.out.println("Existing PK value = " + existingPkValue);

			// remove first 2 alphabetic characters from this
			String prefix = existingPkValue.substring(0, 2);
			String existingPkValueWithPrefixStripped = existingPkValue
					.substring(2);
			...
			int existingIdNum = 0;
			existingIdNum = Integer.parseInt(existingPkValueWithPrefixStripped
					.trim());
			...
			stmt = conn
					.prepareStatement("update _last_used_id set id_value = ? where table_name = ? ");
	...
	String select_configuration_rootPassword() {
		...
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" select c.param_value from _configuration c "
							+ " where c.param_name = ? ");
	...
	boolean update_configuration_rootPassword(String rootPassword) {
		...
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" update _configuration c " +
							" set c.param_value = ? "
							+ " where c.param_name = ? ");
	...
	User select_user(String userId, String password) {
		...
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" select u.* from _user u "
							+ " where user_id = ? "
							+ " and password = ? ");
	...
	ArrayList<User> select_users(String departmentName) {
		...
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" select u.user_id from _user u "
							+ " where department = ? ");
	...
	boolean insert_user(
		...
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" insert into _user "
							+ " (user_id, user_name,user_role,department,password) "
							+ " values(?,?,?,?,?) ");
	...
	boolean update_user(
		...
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" update _user " +
							" set user_name = ?, user_role = ?, department = ?, password = ? " +
							" where user_id = ? ");
	...
	boolean delete_user(
		...
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" delete from _user " +
							" where user_id = ? ");
	...
	Notice select_notice(String noticeId) {
		...
		try {
			PreparedStatement stmt = conn.prepareStatement(
					" select datediff(curdate(),n.date),n.*,u.user_name " +
					" from _notice n join _user u " +
	...
	ArrayList<Notice> select_notices(ArrayList<String> ids) {
		...
		try {
			...
			PreparedStatement stmt = conn.prepareStatement(
					" select datediff(curdate(),n.date),n.* from _notice n " +
							"where n.id in ( " + arguments + " ) " +
	...
	boolean insert_notice(
		...
		boolean success = false;
		try {
			// insert a new record into _registered_author
			PreparedStatement stmt = conn
					.prepareStatement(" insert into _notice "
							+ " (id,title,body,date,user_id,department ) "
							+ " values(?,?,?,?,?,?) ");
	...
	boolean update_notice(
		...
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" update _notice " +
	...
	boolean delete_notice(
			String noticeId) {
		// TODO Auto-generated method stub
		boolean success = false;
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" delete from _notice " +
							" where id = ? ");
	...
	boolean insert_attachment(
		...
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" insert into _attachment "
							+ " (id, file_name,content_type, file_blob, notice_id) "
							+ " values(?,?,?,?,?) ");
			...
			stmt.setBinaryStream(4, (InputStream)is, (int)file.length());
	...
	boolean delete_attachments(
			String noticeId) {
		// TODO Auto-generated method stub
		boolean success = false;
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" delete from _attachment " +
							" where notice_id = ? ");
	...
	boolean checkIfExists_department(
			String name) {
		// TODO Auto-generated method stub
		boolean exists = false;
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" select d.* from _department d "
							+ " where d.name = ? " );
	...
	boolean insert_department(
			String departmentName) {
		// TODO Auto-generated method stub
		boolean success = false;
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" insert into _department "
							+ " (name) "
							+ " values(?) ");
	...
	boolean delete_department(
			String departmentName) {
		// TODO Auto-generated method stub
		boolean success = false;
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" delete from _department " +
							" where name = ? ");
	...
	boolean update_department(
		...
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" update _department " +
							" set name = ? where name = ? ") ;
	...
	boolean update_quickLink(
		...
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" update _quick_link " +
							" set name = ?, url = ? where name = ? ") ;
	...
	boolean delete_quickLink(
			String linkName) {
		// TODO Auto-generated method stub
		boolean success = false;
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" delete from _quick_link " +
							" where name = ? ");
	...
	boolean checkIfExists_quickLink(
			String name) {
		// TODO Auto-generated method stub
		boolean exists = false;
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" select ql.* from _quick_link ql "
							+ " where ql.name = ? " );
	...
	boolean insert_quickLink(
		...
		try {
			PreparedStatement stmt = conn
					.prepareStatement(" insert into _quick_link "
							+ " (name, url) "
							+ " values(?,?) ");
