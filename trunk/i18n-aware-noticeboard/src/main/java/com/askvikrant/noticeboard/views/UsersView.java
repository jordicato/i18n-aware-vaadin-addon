...
public class UsersView extends VerticalLayout implements View{
	...
	private final BeanItemContainer<User> container = 
	...
	public UsersView(final User u){
		...
		btnSave.addClickListener(new ClickListener(){
			...
			public void buttonClick(ClickEvent event) {
				// TODO Auto-generated method stub
				if(tfUserId.getValue().length()!=0
						&& tfUserName.getValue().length()!=0
						&& pfPassword.getValue().length()!=0
						&& combo.getValue().toString().length()!=0){
					BusinessLogic bl = ((NoticeboardUI)getUI()).getBusinessLogic();
					boolean success = bl.saveNewUser(
							tfUserId.getValue(), 
							tfUserName.getValue(), 
							pfPassword.getValue(), 
							User.ROLE_EXECUTIVE, 
