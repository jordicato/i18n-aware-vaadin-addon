...
public class BusinessLogic {
	...
	public boolean saveNewUser(final String userId, final String userName, final String password, final String role, final String department) {
		...
		try {
			...
			if (success) {
				...
				if (success) {
					success = databaseManager.insert_user(userId, userName, password, role, department);
				}
	...
	public boolean updateUserPassword(final String userId, final String oldPassword, final String password1, final String password2) {
			...
			if (user != null) {
				if (password1 != null && password1.length() != 0 && password2 != null && password2.length() != 0) {
					if (password1.equals(password2)) {
						...
						success = false;
						databaseManager.setDbMessage("The two passwords do not match.");
					}
				}
				else {
					success = false;
				...
				success = false;
				databaseManager.setDbMessage("Current password provided is incorrect. Please try again");
			}
		}
		catch (Exception ex) {
			// log the exception
	...
	public boolean updateUser(final String userId, final String userName, final String role, final String password, final String department) {
		...
		try {
			...
			if (success) {
				...
				if (success) {
					success = databaseManager.update_user(userId, userName, password, role, department);
				}
	...
	private ArrayList<String> getNoticeIds(NoticeSearchParams searchParams) {
		...
		try {
			...
			// log the exception
			System.out.println("ERROR: BL: getNoticeIds(): " + ex.getMessage());
		}
		finally {
			databaseManager.closeConnection();
		}
	...
	public boolean saveNewNotice(final String title, final String body, final ArrayList<Attachment> attachments, final String userId,
		...
		try {
			...
			if (success) {
				...
				if (success) {
					...
					success = databaseManager.insert_notice(noticeId, title, body, dateFormat.format(date), userId, department);
				}
				...
				if (success) {
					for (int i = 0; i < attachments.size(); i++ ) {
						...
						if (attachmentId == null) {
							...
							success = databaseManager.insert_attachment(attachmentId, attachments.get(i).getFile().getName(), attachments.get(i)
									.getContentType(), attachments.get(i).getFile(), noticeId);
							if (!success) {
								break;
							}
						}
	...
	public boolean updateNotice(final String noticeId, final String title, final String body, final ArrayList<Attachment> attachments) {
		...
		try {
			...
			if (success) {
				...
				success = databaseManager.update_notice(noticeId, title, body);
	...
	public boolean deleteNotice(final String noticeId) {
	...
	public boolean saveNewDepartment(final String departmentName) {
		boolean success = false;
		try {
			databaseManager.resetDbMessage();
			if (!databaseManager.getConnection()) {
			...
			if (success) {
				...
				if (success) {
					success = databaseManager.insert_department(departmentName);
				}
	...
	public boolean updateDepartment(String oldDepartmentName, String newDepartmentName) {
		...
		try {
			...
			if (success) {
				...
				if (success) {
					success = databaseManager.update_department(oldDepartmentName, newDepartmentName);
				}
	...
	public boolean saveNewQuickLink(final String linkName, final String linkUrl) {
		boolean success = false;
		try {
			databaseManager.resetDbMessage();
			if (!databaseManager.getConnection()) {
			...
			if (success) {
				...
				if (success) {
					success = databaseManager.insert_quickLink(linkName, linkUrl);
				}
	...
	public boolean updateQuickLink(final String oldName, final String newName, final String url) {
		...
		try {
			...
			if (success) {
				...
				if (success) {
					success = databaseManager.update_quickLink(oldName, newName, url);
				}
	...
	public boolean deleteQuickLink(final String linkName) {
	...
	public boolean updateDefaultDays(final String defaultDays) {
