...
public class DatabaseManager {
	...
	boolean insert_user(String userId, String userName, String password, String role, String departmentName) {
		...
		boolean success = false;
		try {
			PreparedStatement stmt = conn.prepareStatement(" insert into _user " + " (user_id, user_name,user_role,department,password) "
					+ " values(?,?,?,?,?) ");
	...
	boolean update_user(String userId, String userName, String password, String role, String departmentName) {
		...
		boolean success = false;
		try {
			PreparedStatement stmt = conn.prepareStatement(" update _user " + " set user_name = ?, user_role = ?, department = ?, password = ? "
					+ " where user_id = ? ");
	...
	Notice select_notice(String noticeId) {
		...
		Notice notice = null;
		try {
			PreparedStatement stmt = conn.prepareStatement(" select datediff(curdate(),n.date),n.*,u.user_name " + " from _notice n join _user u "
					+ " on n.user_id = u.user_id " + "where n.id = ? ");
	...
	ArrayList<Notice> select_notices(ArrayList<String> ids) {
		...
		try {
			...
			PreparedStatement stmt = conn.prepareStatement(" select datediff(curdate(),n.date),n.* from _notice n " + "where n.id in ( " + arguments
	...
	boolean insert_notice(String noticeId, String title, String body, String date, String userId, String department) {
		...
		try {
			...
			PreparedStatement stmt = conn.prepareStatement(" insert into _notice " + " (id,title,body,date,user_id,department ) "
					+ " values(?,?,?,?,?,?) ");
	...
	boolean insert_attachment(String attachmentId, String fileName, String contentType, File file, String noticeId) {
		...
		boolean success = false;
		try {
			PreparedStatement stmt = conn.prepareStatement(" insert into _attachment " + " (id, file_name,content_type, file_blob, notice_id) "
					+ " values(?,?,?,?,?) ");
