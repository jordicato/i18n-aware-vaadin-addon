...
public class UsersView extends VerticalLayout implements View {
	...
	private final Button btnNew = new Button("Add New User");
	...
	private final TextField tfUserId = new TextField("User Id");
	private final TextField tfUserName = new TextField("Name");
	private final PasswordField pfPassword = new PasswordField("Password");
	private final ComboBox combo = new ComboBox("Department");
	...
	public UsersView(final User u) {
		...
		btnSave.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				// TODO Auto-generated method stub
				if (tfUserId.getValue().length() != 0 && tfUserName.getValue().length() != 0 && pfPassword.getValue().length() != 0
						&& combo.getValue().toString().length() != 0) {
					BusinessLogic bl = ((NoticeboardUI) getUI()).getBusinessLogic();
					boolean success = bl.saveNewUser(tfUserId.getValue(), tfUserName.getValue(), pfPassword.getValue(), User.ROLE_EXECUTIVE,
		...
		btnUpdate.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if (tfUserId.getValue().length() != 0 && tfUserName.getValue().length() != 0 && pfPassword.getValue().length() != 0
						&& combo.getValue().toString().length() != 0) {
					BusinessLogic bl = ((NoticeboardUI) getUI()).getBusinessLogic();
					boolean success = bl.updateUser(tfUserId.getValue(), tfUserName.getValue(), User.ROLE_EXECUTIVE, pfPassword.getValue(),
							(String) combo.getValue());
	...
	private void load() {
		// invoke business logic method to fetch departments & users
		BusinessLogic bl = ((NoticeboardUI) getUI()).getBusinessLogic();
