...
//		tfTitle.setRequiredError("Please provide an appropriate title");

		taBody.setWidth(100, Unit.PERCENTAGE);
		taBody.setHeight(120, Unit.PIXELS);
		taBody.setInputPrompt("Message body");
//		taBody.setRequired(true);
//		taBody.setRequiredError("Please provide message body");
		...
		btnPost.addClickListener(new ClickListener(){
			...
			public void buttonClick(ClickEvent event) {
				...
				if(tfTitle.getValue().length()!=0
						&& taBody.getValue().length()!=0){
					BusinessLogic bl = ((NoticeboardUI)getUI()).getBusinessLogic();
					boolean success = bl.saveNewNotice(
							tfTitle.getValue(), 
							taBody.getValue(), 
							filesPanel.getAttachments(),
							user.getUserId(), 
							user.getDepartment());
		...
		btnUpdate.addClickListener(new ClickListener(){
			...
			public void buttonClick(ClickEvent event) {
				...
				if(tfTitle.getValue().length()!=0
						&& taBody.getValue().length()!=0){
					BusinessLogic bl = ((NoticeboardUI)getUI()).getBusinessLogic();
					boolean success = bl.updateNotice(
							noticeToEdit.getId(), 
							tfTitle.getValue(), 
							taBody.getValue(), 
							filesPanel.getAttachments());
