	...
	public PasswordEditorView(final User u){
		...
		btnUpdate.addClickListener(new ClickListener(){
			...
			public void buttonClick(ClickEvent event) {
				...
				if(tfOldPassword.getValue().length()!=0
						&& tfNewPassword1.getValue().length()!=0
						&& tfNewPassword2.getValue().length()!=0){
					BusinessLogic bl = ((NoticeboardUI)getUI()).getBusinessLogic();
					boolean success = bl.updateUserPassword(
							user.getUserId(), 
							tfOldPassword.getValue(), 
							tfNewPassword1.getValue(), 
