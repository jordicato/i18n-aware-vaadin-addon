...
public final class ExampleUtil {
    ...
    private static final String[][] locales = { { "fi", "FI", "Finnish" }, { "de", "DE", "German" }, { "en", "US", "US - English" }, { "sv", "SE", "Swedish" } };

    private static final String[][] hardware = { { "Desktops", "Dell OptiPlex GX240", "Dell OptiPlex GX260", "Dell OptiPlex GX280" }, { "Monitors", "Benq T190HD", "Benq T220HD", "Benq T240HD" }, { "Laptops", "IBM ThinkPad T40", "IBM ThinkPad T43", "IBM ThinkPad T60" } };

    public static final Object timezone_PROPERTY_TIME_ZONE = "timeZone";

    public static final Object timezone_PROPERTY_NAME = "name";

    private static final String[] timezones = { "UTC", "Europe/Helsinki", "Australia/Melbourne", "America/New_York" };
    ...
    private static final String[] firstnames = new String[] { "John", "Mary", "Joe", "Sarah", "Jeff", "Jane", "Peter", "Marc", "Robert", "Paula", "Lenny", "Kenny", "Nathan", "Nicole", "Laura", "Jos", "Josie", "Linus" };

    private static final String[] lastnames = new String[] { "Torvalds", "Smith", "Adams", "Black", "Wilson", "Richards", "Thompson", "McGoff", "Halas", "Jones", "Beck", "Sheridan", "Picard", "Hill", "Fielding", "Einstein" };
    ...
    public static IndexedContainer getNameContainer() {
        IndexedContainer contactContainer = new IndexedContainer();
        contactContainer.addContainerProperty(PERSON_PROPERTY_NAME, String.class, "");
        Random r = new Random(5);
        for (int i = 0; i < 50; ) {
            String fn = firstnames[(int) (r.nextDouble() * firstnames.length)];
            String ln = lastnames[(int) (r.nextDouble() * lastnames.length)];
            ...
            if (item != null) {
                i++;
                item.getItemProperty(PERSON_PROPERTY_NAME).setValue(fn + " " + ln);
            }
        }
        return contactContainer;
    }
    ...
    public static IndexedContainer getTimeZoneContainer() {
        IndexedContainer localeContainer = new IndexedContainer();
        localeContainer.addContainerProperty(timezone_PROPERTY_TIME_ZONE, TimeZone.class, null);
        localeContainer.addContainerProperty(timezone_PROPERTY_NAME, String.class, null);
        for (int i = 0; i < timezones.length; i++) {
            String id = timezones[i];
            Item item = localeContainer.addItem(id);
            TimeZone timeZone = TimeZone.getTimeZone(id);
            item.getItemProperty(timezone_PROPERTY_TIME_ZONE).setValue(timeZone);
            item.getItemProperty(timezone_PROPERTY_NAME).setValue(timeZone.getDisplayName());
        }
        return localeContainer;
    }
    ...
    private static void fillIso3166Container(IndexedContainer container) {
        ...
        for (int i = 0; i < iso3166.length; i++) {
            ...
            item.getItemProperty(iso3166_PROPERTY_FLAG).setValue(new ThemeResource("../sampler/flags/" + id.toLowerCase() + ".gif"));
        }
    ...
    public static IndexedContainer getOrderContainer() {
        ...
        container.addContainerProperty(ORDER_UNITPRICE_PROPERTY_ID, String.class, "$0");
        container.addContainerProperty(ORDER_ITEMPRICE_PROPERTY_ID, String.class, "$0");
    ...
    private static void addOrderToContainer(Container container, String description, int quantity, double price) {
        ...
        item.getItemProperty(ORDER_UNITPRICE_PROPERTY_ID).setValue(NumberFormat.getCurrencyInstance().format(price));
        item.getItemProperty(ORDER_ITEMPRICE_PROPERTY_ID).setValue(NumberFormat.getCurrencyInstance().format(price * quantity));
    }
}
